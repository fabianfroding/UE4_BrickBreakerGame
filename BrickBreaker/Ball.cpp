...
ABall::ABall()
{
	...
	ProjectileMovement->Friction = 0.0f; // Friction when colliding with other 3d objects.
...
void ABall::Launch()
{
	if (!BallLaunched)
	{
		// Second param: In which part of body to add impulse. FName() = whole body
		// Third param: Should we change velocity when we change impulse?
